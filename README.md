...
```bash
venv/bin/python3 ecs_csv_quoter_auto.py /path/to/file.csv -t <用户open_id>
```

**场景二：文本配置报价**

用户提供的文本描述、非标准文件等，先分析统计规格，确认后报价。

```bash
venv/bin/python3 ecs_text_quoter.py '配置内容' --region cn-hangzhou -t <用户open_id>
```
...
分析 OSS Bucket 使用情况，生成统计报告。

```bash
venv/bin/python3 oss_quoter_auto.py <AccessKey> <SecretKey> -t <用户open_id>
```
...
| `oss2` | >=2.19.0 | OSS SDK（可选，仅 OSS 统计需要） |
