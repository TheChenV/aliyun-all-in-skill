...
# -*- coding: utf-8 -*-
"""
阿里云 ECS 报价技能 - CSV 自动报价入口
调用 ecs_csv_quoter.py 生成报价单，输出文件路径供 AI 发送
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description="ECS 报价单自动生成（CSV 场景）")
    ...
    # 输出结果（供 AI 读取）
    if instance_count > 0:
        print(f"\n✅ 报价单已生成：{excel_path}（共 {instance_count} 台实例）")
    else:
        print(f"\n✅ 报价单已生成：{excel_path}")
