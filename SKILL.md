...
当用户输入文字描述配置（而非 CSV 文件）时，AI 需要先**标准化**配置文本，再调用脚本。脚本的正则解析依赖标准格式，直接传入用户原文会导致数据丢失。

### 标准化步骤

1. **识别每台实例** — 按序号分割（1. / 2. / 3. ...）
2. **提取每项配置** — 地域、规格、镜像、系统盘、数据盘、带宽
3. **补全缺失字段** — 用户未明确提供的字段按「默认值规则」补全
4. **统一格式** — 按以下标准格式重组后传给脚本

### 标准输入格式

```
1. <地域> 实例规格：<规格代码> (<vCPU> vCPU <内存> GiB) 镜像：<镜像名称> 系统盘：<类型> <大小>GiB 数据盘：<类型> <大小>GiB 公网带宽：<计费方式> <大小>Mbps
2. ...
```
...
⚠️ **收费镜像**：Windows 在中国香港及海外地域收费，内地免费。Red Hat / SUSE / Alibaba Cloud Linux 3 Pro 全球收费。遇到收费镜像需告知用户「不报价，需人工确认」。

⚠️ **地域**：每台实例可独立指定地域。用户未指定时默认杭州（cn-hangzhou）。**不要把某台实例的地域当成全局地域**。

⚠️ **磁盘**：用户只写大小没写类型 → 默认 ESSD PL0。只写一个大小 → 系统盘。写多个大小 → 最小的为系统盘，其余为数据盘。

⚠️ **带宽**：用户写了大小但没写计费方式 → ≤20Mbps 默认按固定带宽，≥21Mbps 默认按流量计费。
...
AI 按上述「AI 执行指引」标准化配置后，调用：

```bash
venv/bin/python3 scripts/ecs_text_quoter.py '<标准化后的配置文本>'
```
...
| 配置项 | 默认值 | 判断条件 |
|--------|--------|----------|
| 地域 | cn-hangzhou（杭州） | 用户未指定时 |
| 磁盘类型 | ESSD PL0 | 用户只写了大小，未指定类型 |
| 系统盘大小 | 40 GiB | 用户未提供任何磁盘大小 |
| 带宽计费方式 | PayByBandwidth（固定带宽） | 用户提供带宽 ≤20Mbps，未说明计费方式 |
| 带宽计费方式 | PayByTraffic（按流量计费） | 用户提供带宽 ≥21Mbps，未说明计费方式 |
| 镜像 | 公共免费镜像 | 使用 CentOS / Ubuntu / Alibaba Cloud Linux 等免费 OS |
...
| 步骤 | 操作 | 说明 |
|------|------|------|
| 0 | 输入标准化 | **仅场景二**：AI 先将用户原文标准化为标准格式（见上方「AI 执行指引」） |
| 1 | 调用脚本 | 根据场景选择对应脚本 |
| 2 | 输出路径 | 脚本输出 `FILE_PATH:xxx.xlsx`，AI 读取此路径 |
| 3 | 发送文件 | 钉钉渠道用裸露路径独占一行，其他渠道用 `openclaw message send` CLI（见下方「文件发送⚠️」） |
| 4 | 无需清理 | **不要删除文件！** 钉钉连接器异步上传，AI 删得太快会导致上传失败。文件默认长期保留，由用户手动定期清理。 |
...
- **钉钉连接器是异步上传的，AI 绝不要在发送路径后删除文件！** 否则连接器读取时文件已不存在
- 文件默认长期保留，由用户手动定期清理，AI 无需操作
